...
__m256i dot(uint32_t n, int32_t *vec1, int32_t *vec2)  {
  ...
  // }
  // //dot_prod += tmp_arr[0] + tmp_arr[1] + tmp_arr[2] +tmp_arr[3] + tmp_arr[4] + tmp_arr[5] + tmp_arr[6] + tmp_arr[7];
...
matrix_t* create_matrix(uint32_t rows, uint32_t cols) {
  ...
  new->rows = rows;
  new->cols = cols;
  new->data = malloc(((rows * cols) + 8) * sizeof(uint32_t));
